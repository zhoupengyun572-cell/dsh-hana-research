...
const HANA_ROUTE_TITLES = {
  literature: '文献中心 · HanaResearch',
  projects: '项目库 · HanaResearch',
};
const routeScrollMemory = { literature: 0, projects: 0 };
// SPA 自管滚动记忆（路由切换 + 硬刷新恢复），关掉浏览器原生恢复避免双重跳位竞争
...
async function navigateWorkspace(route, { push = true } = {}) {
  ...
  document.title = HANA_ROUTE_TITLES[route] || document.title;
  const savedScroll = routeScrollMemory[route] || 0;
  routeScrollMemory[route] = 0;
  // 回调返回 promise：View Transition 在渲染完成后才采样新快照，避免转场动画定格在骨架屏
  const runSwap = async () => {
    window.scrollTo(0, 0);
    updateAppBarRoute();
    await (route === 'projects' ? loadProjects() : loadLiterature());
...
updateAppBarRoute();
document.title = HANA_ROUTE_TITLES[workspace] || document.title;
if (workspace === 'projects') {
  loadProjects();
} else {
  // 硬刷新恢复上次筛选/滚动现场（pagehide 写入；仅在 boot 消费一次，
  // SPA 内切换不读它，避免旧值覆盖内存中的筛选态）
  let bootScrollY = 0;
  try {
    const saved = JSON.parse(safeStorage.sessionGet('hana-literature-state') || 'null');
    if (saved) {
      if (saved.query !== undefined) state.query = String(saved.query);
      if (saved.activeTopic) state.activeTopic = String(saved.activeTopic);
      if (saved.activeVenue) state.activeVenue = String(saved.activeVenue);
      if (saved.activeReadStatus) state.activeReadStatus = String(saved.activeReadStatus);
      if (saved.activeMethodology) state.activeMethodology = String(saved.activeMethodology);
      bootScrollY = Number(saved.scrollY) || 0;
    }
  } catch { /* 忽略恢复失败 */ }
  loadLiterature().then(() => { if (bootScrollY) restoreRouteScroll('literature', bootScrollY); });
}
