...
- 常驻顶栏激活：品牌区（文献研究台）+「文献中心/项目库」分段切换器 + 命令面板/设置工具组，sticky 毛玻璃跟随宿主主题，窄屏自动紧凑
- **SPA 路由**：两个工作区在同一文档内切换——pushState + View Transitions 形变过渡，无整页白屏重载；浏览器前进/后退可用；深链接与旧书签完全兼容（服务端 data-workspace 初始化不变）；Ctrl/Cmd/中键点击仍由浏览器新开标签
