// Client half of dsh-hana-research — hand-written bundle in the exact
...
// 注册 id 必须与 client-modules graph row 的 id（安装包名）一致：
// junction 开发安装是 "@local/dsh-hana-research"，npm 正式安装是 "dsh-hana-research"。
// 壳的 boot manifest（__DSH_BOOT__.entries）按 url 列出每个 client bundle 的 id，
// 这里按自己的包名反查，兼容两种安装形态（注入执行下 currentScript 不可用）。
var hanaPluginIds = (function () {
	var fallback = { id: "dsh-hana-research" };
	try {
		var boot = (typeof window !== "undefined" && window.__DSH_BOOT__) || null;
		var entries = boot && Array.isArray(boot.entries) ? boot.entries : [];
		var ids = [];
		for (var i = 0; i < entries.length; i += 1) {
			var entry = entries[i];
			if (entry && typeof entry.url === "string" && entry.url.indexOf("dsh-hana-research/client.js") !== -1 && ids.indexOf(entry.id) === -1) {
				ids.push(entry.id);
			}
		}
		return ids.length ? ids : [fallback.id];
	} catch {
		return [fallback.id];
	}
})();
for (var __hanaIdIndex = 0; __hanaIdIndex < hanaPluginIds.length; __hanaIdIndex += 1) {
window.__ModuleLoader__.load({
	id: hanaPluginIds[__hanaIdIndex],
	factory: (require) => {
		var module = { exports: {} };
