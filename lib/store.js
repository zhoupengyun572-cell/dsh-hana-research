...
import { FULLTEXT_STATUSES, buildMatchQuery, restoreCjkSpacing } from './fulltext-index.js';
...
export const RESEARCH_SCHEMA_VERSION = 22;
...
export class ResearchStore {
  ...
  constructor(dataDir, options = {}) {
    ...
    // 项目笔记 md 文件的延迟合并写（P0-4 写放大修复）：同一时间窗内的多次笔记变更只落盘一次
    this.#pendingNotesSync = new Set();
    this.#notesSyncTimer = null;
  }
  ...
  #initSchema() {
    this.db.exec(`
      ...
      /* 期刊订阅同步/期刊页计数按 venue+source_name 关联 papers，无索引时每次 GET /journals 全表扫描 */
      CREATE INDEX IF NOT EXISTS idx_papers_venue_source ON papers(venue, source_name);
    ...
    // ── schema v20 迁移（AI 预筛：第三评审运行与结果，独立于双人工筛选口径） ──
    if (currentVersion >= 19 && currentVersion < 20 && fs.existsSync(this.dbPath)) {
      try {
        this.db.exec('PRAGMA wal_checkpoint(TRUNCATE)');
        const stamp = nowIso().replace(/[:.]/g, '-');
        const backupPath = `${this.dbPath}.bak-v20-${stamp}`;
        fs.copyFileSync(this.dbPath, backupPath);
        this.#audit('schema.backup', 'database', backupPath, { from: currentVersion, to: 20 });
      } catch (error) {
        console.warn('[hana-research] 数据库备份失败（v20 迁移继续）:', error?.message || error);
      }
    }
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS ai_screening_runs (
        ...
        target_paper_ids TEXT NOT NULL DEFAULT '[]',
        ...
        last_paper_id TEXT NOT NULL DEFAULT '',
        error TEXT NOT NULL DEFAULT '',
        created_at TEXT NOT NULL,
        updated_at TEXT NOT NULL,
        FOREIGN KEY (project_id) REFERENCES projects(id) ON DELETE CASCADE
      );
      CREATE INDEX IF NOT EXISTS idx_ai_runs_project ON ai_screening_runs(project_id, created_at DESC);

      CREATE TABLE IF NOT EXISTS paper_ai_screenings (
        id TEXT PRIMARY KEY,
        ...
        rationale TEXT NOT NULL DEFAULT '',
        per_criteria TEXT NOT NULL DEFAULT '[]',
        tier INTEGER NOT NULL CHECK(tier BETWEEN 1 AND 3),
        sent_scope TEXT NOT NULL DEFAULT 'metadata' CHECK(sent_scope IN ('metadata', 'fulltext_excerpt')),
        created_at TEXT NOT NULL,
        UNIQUE(run_id, paper_id),
        FOREIGN KEY (run_id) REFERENCES ai_screening_runs(id) ON DELETE CASCADE,
        FOREIGN KEY (project_id, paper_id) REFERENCES project_papers(project_id, paper_id) ON DELETE CASCADE
      );
      CREATE INDEX IF NOT EXISTS idx_ai_screenings_project_stage ON paper_ai_screenings(project_id, stage, tier);
    `);

    // ── schema v21 迁移（全文检索：FTS5 虚表 + 附件索引状态；索引只含本地 PDF 内容，不外发） ──
    // v20 从未随版本发布（0.4.0-beta.1 = v19），真实升级路径是 v19→v21：备份条件覆盖全部未发布跳段。
    if (currentVersion >= 14 && currentVersion < 21 && fs.existsSync(this.dbPath)) {
      try {
        this.db.exec('PRAGMA wal_checkpoint(TRUNCATE)');
        const stamp = nowIso().replace(/[:.]/g, '-');
        const backupPath = `${this.dbPath}.bak-v21-${stamp}`;
        fs.copyFileSync(this.dbPath, backupPath);
        this.#audit('schema.backup', 'database', backupPath, { from: currentVersion, to: 21 });
      } catch (error) {
        console.warn('[hana-research] 数据库备份失败（v21 迁移继续）:', error?.message || error);
      }
    }
    this.db.exec(`
    ...
    `);
    const attachmentFtsColumns = new Set(this.db.prepare('PRAGMA table_info(attachments)').all().map(column => column.name));
    if (!attachmentFtsColumns.has('fts_status')) this.db.exec("ALTER TABLE attachments ADD COLUMN fts_status TEXT NOT NULL DEFAULT 'pending'");
    if (!attachmentFtsColumns.has('fts_indexed_at')) this.db.exec('ALTER TABLE attachments ADD COLUMN fts_indexed_at TEXT');
    if (!attachmentFtsColumns.has('fts_page_count')) this.db.exec('ALTER TABLE attachments ADD COLUMN fts_page_count INTEGER');

    // ── schema v22 迁移（引用核验：汇总笔记引文的四态审计账本） ──
    if (currentVersion >= 14 && currentVersion < 22 && fs.existsSync(this.dbPath)) {
      try {
        this.db.exec('PRAGMA wal_checkpoint(TRUNCATE)');
        const stamp = nowIso().replace(/[:.]/g, '-');
        const backupPath = `${this.dbPath}.bak-v22-${stamp}`;
        fs.copyFileSync(this.dbPath, backupPath);
        this.#audit('schema.backup', 'database', backupPath, { from: currentVersion, to: 22 });
      } catch (error) {
        console.warn('[hana-research] 数据库备份失败（v22 迁移继续）:', error?.message || error);
      }
    }
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS citation_verifications (
        ...
        paper_id TEXT NOT NULL,
        status TEXT NOT NULL CHECK(status IN ('verified', 'mismatch', 'not_found', 'manual_needed')),
        conflicts_json TEXT NOT NULL DEFAULT '[]',
        sources_json TEXT NOT NULL DEFAULT '[]',
        checked_at TEXT NOT NULL,
        created_at TEXT NOT NULL,
        FOREIGN KEY (citation_id) REFERENCES note_citations(id) ON DELETE CASCADE,
        FOREIGN KEY (paper_id) REFERENCES papers(id) ON DELETE CASCADE
      );
      CREATE INDEX IF NOT EXISTS idx_citation_verifications_note ON citation_verifications(citation_id);
    `);
  ...
  getProject(projectId) {
    // 单行直查：写作路径高频调用（≥29 处），不能经由 listProjects 全表聚合后 find
  ...
  getPaper(paperId) {
    // 单行直查：与 listPapers 同形状（MIN(a.id)/GROUP_CONCAT 聚合），仅限单 id
    const row = this.db.prepare(`
      SELECT p.*,
        MIN(a.id) AS attachment_id,
        GROUP_CONCAT(DISTINCT pp.project_id) AS imported_project_ids
  ...
  // ── AI 预筛（v20：第三评审。结果独立存储，不写入 paper_screening_reviews；PRISMA 口径不受影响） ──

  #aiScreeningCriteriaSnapshot(projectId) {
    const payload = this.listScreeningCriteria(projectId)
      .filter(item => item.enabled)
      .map(item => ({ kind: item.kind, label: item.label, description: item.description, position: item.position }));
    const criteriaHash = crypto.createHash('sha256').update(JSON.stringify(payload)).digest('hex').slice(0, 16);
    return { criteriaHash, criteriaSnapshot: payload };
  }

  #aiRunRowToView(row) {
    let criteriaSnapshot = [];
    try { criteriaSnapshot = JSON.parse(row.criteria_snapshot || '[]'); } catch { criteriaSnapshot = []; }
    let targetPaperIds = [];
    try { targetPaperIds = JSON.parse(row.target_paper_ids || '[]'); } catch { targetPaperIds = []; }
    return {
      id: row.id,
      projectId: row.project_id,
      stage: row.stage,
  ...
  #refreshAiRunCounters(runId, stamp) {
    this.db.prepare(`
      UPDATE ai_screening_runs SET
        processed = (SELECT COUNT(*) FROM paper_ai_screenings WHERE run_id = ?),
        included = (SELECT COUNT(*) FROM paper_ai_screenings WHERE run_id = ? AND decision = 'include'),
        excluded = (SELECT COUNT(*) FROM paper_ai_screenings WHERE run_id = ? AND decision = 'exclude'),
        uncertain = (SELECT COUNT(*) FROM paper_ai_screenings WHERE run_id = ? AND decision = 'uncertain'),
        last_paper_id = (SELECT paper_id FROM paper_ai_screenings WHERE run_id = ? ORDER BY created_at DESC, rowid DESC LIMIT 1),
        updated_at = ?
      WHERE id = ?
    `).run(runId, runId, runId, runId, runId, stamp, runId);
  }

  #aiTierFor(decision, confidence) {
    if (decision === 'uncertain') return 3;
    if (confidence >= 0.75) return 1;
    if (confidence >= 0.5) return 2;
    return 3;
  }

  createAiScreeningRun({ projectId, stage, paperIds = null, model = '', promptVersion = '' } = {}) {
    if (!this.getProject(projectId)) throw new ResearchStoreError('PROJECT_NOT_FOUND', '项目不存在', 404);
    if (!['title_abstract', 'full_text'].includes(stage)) throw new ResearchStoreError('SCREENING_STAGE_INVALID', '筛选阶段无效', 400);
    const { criteriaHash, criteriaSnapshot } = this.#aiScreeningCriteriaSnapshot(projectId);
    if (!criteriaSnapshot.length) throw new ResearchStoreError('AI_SCREENING_CRITERIA_REQUIRED', '请先配置纳排标准再运行 AI 预筛', 400);
    const papers = this.listProjectPapers(projectId);
    const decisionField = stage === 'title_abstract' ? 'titleAbstractDecision' : 'fullTextDecision';
    let targets;
    if (paperIds === null || paperIds === undefined) {
      targets = papers.filter(paper => paper[decisionField] === 'pending');
    } else {
      const ids = [...new Set((Array.isArray(paperIds) ? paperIds : []).map(value => String(value || '')).filter(Boolean))];
      if (!ids.length || ids.length > 500) throw new ResearchStoreError('AI_SCREENING_BATCH_INVALID', '请选择 1–500 篇项目文献', 400);
      const known = new Set(papers.map(paper => paper.id));
      if (ids.some(id => !known.has(id))) throw new ResearchStoreError('PROJECT_PAPER_NOT_FOUND', '所选文献中有项目外文献', 404);
      targets = papers.filter(paper => ids.includes(paper.id));
    }
    if (!targets.length) throw new ResearchStoreError('AI_SCREENING_NO_TARGETS', '该阶段没有待筛文献', 400);
    const id = crypto.randomUUID();
    const stamp = nowIso();
    this.db.prepare(`
      INSERT INTO ai_screening_runs(id, project_id, stage, criteria_hash, criteria_snapshot, target_paper_ids, model, prompt_version, status, total, created_at, updated_at)
      VALUES(?, ?, ?, ?, ?, ?, ?, ?, 'queued', ?, ?, ?)
    `).run(id, projectId, stage, criteriaHash, JSON.stringify(criteriaSnapshot), JSON.stringify(targets.map(paper => paper.id)),
      String(model || ''), String(promptVersion || ''), targets.length, stamp, stamp);
    this.#audit('screening.ai.run.create', 'project', projectId, { runId: id, stage, total: targets.length });
    return this.getAiScreeningRun(projectId, id);
  }

  getAiScreeningRun(projectId, runId) {
    const row = this.db.prepare('SELECT * FROM ai_screening_runs WHERE project_id = ? AND id = ?').get(projectId, runId);
    if (!row) throw new ResearchStoreError('AI_SCREENING_RUN_NOT_FOUND', 'AI 预筛任务不存在', 404);
    return this.#aiRunRowToView(row);
  }

  listAiScreeningRuns(projectId) {
    if (!this.getProject(projectId)) throw new ResearchStoreError('PROJECT_NOT_FOUND', '项目不存在', 404);
    return this.db.prepare('SELECT * FROM ai_screening_runs WHERE project_id = ? ORDER BY created_at DESC, rowid DESC').all(projectId)
      .map(row => this.#aiRunRowToView(row));
  }

  updateAiScreeningRunStatus({ projectId, runId, status, error = '' } = {}) {
    const allowedFrom = {
      running: ['queued', 'paused', 'failed'],
      paused: ['running'],
      done: ['running'],
      failed: ['running'],
      cancelled: ['queued', 'running', 'paused', 'failed'],
      queued: ['paused', 'failed', 'cancelled'],
    };
    if (!allowedFrom[status]) throw new ResearchStoreError('AI_SCREENING_STATUS_INVALID', 'AI 预筛任务状态无效', 400);
    const run = this.getAiScreeningRun(projectId, runId);
    if (!allowedFrom[status].includes(run.status)) {
      throw new ResearchStoreError('AI_SCREENING_STATUS_INVALID', `任务状态不能从 ${run.status} 变为 ${status}`, 400);
    }
    const stamp = nowIso();
    // 断点续跑：last_paper_id 保留游标，恢复运行时跳过已写入结果（UNIQUE 幂等）
    this.db.prepare('UPDATE ai_screening_runs SET status = ?, error = ?, updated_at = ? WHERE id = ?')
      .run(status, status === 'queued' || status === 'running' ? '' : String(error || '').slice(0, 500), stamp, runId);
    this.#audit('screening.ai.run.status', 'project', projectId, { runId, from: run.status, to: status });
    return this.getAiScreeningRun(projectId, runId);
  }

  appendAiScreeningResult({ runId, paperId, decision, confidence, rationale = '', perCriteria = [], sentScope = 'metadata' } = {}) {
    const runRow = this.db.prepare('SELECT * FROM ai_screening_runs WHERE id = ?').get(runId);
    if (!runRow) throw new ResearchStoreError('AI_SCREENING_RUN_NOT_FOUND', 'AI 预筛任务不存在', 404);
    if (runRow.status !== 'running') throw new ResearchStoreError('AI_SCREENING_RUN_NOT_RUNNING', 'AI 预筛任务未在运行中', 409);
    if (!['include', 'exclude', 'uncertain'].includes(decision)) throw new ResearchStoreError('AI_SCREENING_RESULT_INVALID', 'AI 预筛结论无效', 400);
    const conf = Number(confidence);
    if (!Number.isFinite(conf) || conf < 0 || conf > 1) throw new ResearchStoreError('AI_SCREENING_RESULT_INVALID', '置信度必须是 0–1 的数值', 400);
    const cleanRationale = String(rationale || '').trim().slice(0, 2000);
    if (!['metadata', 'fulltext_excerpt'].includes(sentScope)) throw new ResearchStoreError('AI_SCREENING_RESULT_INVALID', '发送范围无效', 400);
    const criteria = (Array.isArray(perCriteria) ? perCriteria : []).slice(0, 30).map(item => ({
      id: String(item?.id || ''),
      label: String(item?.label || '').trim().slice(0, 100),
      hit: item?.hit === true,
      evidence: String(item?.evidence || '').trim().slice(0, 500),
    }));
    const member = this.db.prepare('SELECT 1 FROM project_papers WHERE project_id = ? AND paper_id = ?').get(runRow.project_id, paperId);
    if (!member) throw new ResearchStoreError('PROJECT_PAPER_NOT_FOUND', '项目中没有该文献', 404);
    const id = crypto.randomUUID();
    const stamp = nowIso();
    this.db.prepare(`
      INSERT INTO paper_ai_screenings(id, run_id, project_id, paper_id, stage, decision, confidence, rationale, per_criteria, tier, sent_scope, created_at)
      VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(run_id, paper_id) DO UPDATE SET
        ...
        per_criteria = excluded.per_criteria, tier = excluded.tier, sent_scope = excluded.sent_scope, created_at = excluded.created_at
    `).run(id, runId, runRow.project_id, String(paperId || ''), runRow.stage, decision, conf, cleanRationale,
      JSON.stringify(criteria), this.#aiTierFor(decision, conf), sentScope, stamp);
    this.#refreshAiRunCounters(runId, stamp);
    return this.listAiScreenings(runRow.project_id, { runId }).find(item => item.paperId === paperId) || null;
  }

  listAiScreenings(projectId, { runId = null, stage = null, tier = null } = {}) {
    if (!this.getProject(projectId)) throw new ResearchStoreError('PROJECT_NOT_FOUND', '项目不存在', 404);
    const conditions = ['pas.project_id = ?'];
    const params = [projectId];
    if (runId) { conditions.push('pas.run_id = ?'); params.push(String(runId)); }
    if (stage) {
      if (!['title_abstract', 'full_text'].includes(stage)) throw new ResearchStoreError('SCREENING_STAGE_INVALID', '筛选阶段无效', 400);
      conditions.push('pas.stage = ?'); params.push(stage);
    }
    if (tier !== null && tier !== undefined) {
      const tierNum = Number(tier);
      if (![1, 2, 3].includes(tierNum)) throw new ResearchStoreError('AI_SCREENING_RESULT_INVALID', 'AI 预筛分层无效', 400);
      conditions.push('pas.tier = ?'); params.push(tierNum);
    }
    return this.db.prepare(`
      SELECT pas.*, papers.title AS paper_title
      FROM paper_ai_screenings pas
      LEFT JOIN papers ON papers.id = pas.paper_id
      WHERE ${conditions.join(' AND ')}
      ORDER BY pas.tier ASC, pas.confidence DESC, pas.created_at ASC
    `).all(...params).map(row => ({
      id: row.id,
      runId: row.run_id,
      projectId: row.project_id,
      ...
      perCriteria: (() => { try { return JSON.parse(row.per_criteria || '[]'); } catch { return []; } })(),
      tier: Number(row.tier || 3),
      sentScope: row.sent_scope || 'metadata',
      createdAt: row.created_at,
    }));
  }

  buildAiScreeningAgreement(projectId, { stage, runId = null } = {}) {
    if (!this.getProject(projectId)) throw new ResearchStoreError('PROJECT_NOT_FOUND', '项目不存在', 404);
    if (!['title_abstract', 'full_text'].includes(stage)) throw new ResearchStoreError('SCREENING_STAGE_INVALID', '筛选阶段无效', 400);
    let run;
    if (runId) {
      run = this.getAiScreeningRun(projectId, runId);
      if (run.stage !== stage) throw new ResearchStoreError('SCREENING_STAGE_INVALID', '筛选阶段与任务不一致', 400);
    } else {
      const row = this.db.prepare(`
        SELECT id FROM ai_screening_runs WHERE project_id = ? AND stage = ? ORDER BY created_at DESC, rowid DESC LIMIT 1
      `).get(projectId, stage);
      if (!row) return { available: false, stage, reason: 'no_runs' };
      run = this.getAiScreeningRun(projectId, row.id);
    }
    const results = this.listAiScreenings(projectId, { runId: run.id, stage });
    const papers = this.listProjectPapers(projectId);
    const decisionField = stage === 'title_abstract' ? 'titleAbstractDecision' : 'fullTextDecision';
    const humanByPaper = new Map(papers.filter(paper => paper[decisionField] !== 'pending').map(paper => [paper.id, paper[decisionField]]));
    // 口径：AI 的 include 视为阳性；人工的 include 视为阳性，maybe 与 exclude 均视为阴性（保守）。
    const counts = { tp: 0, fp: 0, tn: 0, fn: 0 };
    let compared = 0;
    for (const item of results) {
      const human = humanByPaper.get(item.paperId);
      if (human === undefined) continue;
      compared += 1;
      const aiPositive = item.decision === 'include';
      const humanPositive = human === 'include';
      if (aiPositive && humanPositive) counts.tp += 1;
      else if (aiPositive && !humanPositive) counts.fp += 1;
      else if (!aiPositive && humanPositive) counts.fn += 1;
      else counts.tn += 1;
    }
    ...
    return {
      ...
      criteriaHash: run.criteriaHash,
      criteriaChanged: run.criteriaHash !== this.#aiScreeningCriteriaSnapshot(projectId).criteriaHash,
      sampleSize: compared,
      aiDecisions: { include: run.included, exclude: run.excluded, uncertain: run.uncertain },
      counts,
      sensitivity: rate(counts.tp, counts.tp + counts.fn),
      specificity: rate(counts.tn, counts.tn + counts.fp),
      agreementRate: rate(counts.tp + counts.tn, compared),
      overrides: counts.fp + counts.fn,
    };
  }

  // ── 全文检索（v21：FTS5。索引只含本地 PDF 内容，不外发） ──

  updateAttachmentFulltextState({ attachmentId, status, pageCount = null } = {}) {
    if (!FULLTEXT_STATUSES.includes(status)) throw new ResearchStoreError('FULLTEXT_STATUS_INVALID', '全文索引状态无效', 400);
    const result = this.db.prepare('UPDATE attachments SET fts_status = ?, fts_indexed_at = ?, fts_page_count = ? WHERE id = ?')
      .run(status, nowIso(), pageCount, attachmentId);
    if (result.changes === 0) throw new ResearchStoreError('ATTACHMENT_NOT_FOUND', '附件不存在', 404);
  }

  markAttachmentFulltextFailed(attachmentId, error = '') {
    const result = this.db.prepare("UPDATE attachments SET fts_status = 'failed', fts_indexed_at = ?, fts_page_count = 0 WHERE id = ?")
      .run(nowIso(), attachmentId);
    if (result.changes === 0) throw new ResearchStoreError('ATTACHMENT_NOT_FOUND', '附件不存在', 404);
    this.#audit('fulltext.index.failed', 'attachment', attachmentId, { error: String(error || '').slice(0, 300) });
  }

  /** 整体替换一个附件的 FTS 行（pages.text 需已做 CJK 切分），并落 ready/empty 状态。 */
  replaceAttachmentFulltext({ attachmentId, paperId, pages } = {}) {
    const attachment = this.db.prepare('SELECT id, paper_id FROM attachments WHERE id = ?').get(attachmentId);
    if (!attachment) throw new ResearchStoreError('ATTACHMENT_NOT_FOUND', '附件不存在', 404);
    const items = (Array.isArray(pages) ? pages : [])
      .map(item => ({ page: Number(item?.page || 0), text: String(item?.text || '') }))
      .filter(item => item.text.trim());
    withTransaction(this.db, () => {
      this.db.prepare('DELETE FROM papers_fts WHERE attachment_id = ?').run(attachmentId);
      const insert = this.db.prepare('INSERT INTO papers_fts(content, paper_id, attachment_id, page) VALUES(?, ?, ?, ?)');
      for (const item of items) insert.run(item.text, paperId, attachmentId, item.page);
      this.db.prepare("UPDATE attachments SET fts_status = ?, fts_indexed_at = ?, fts_page_count = ? WHERE id = ?")
        .run(items.length ? 'ready' : 'empty', nowIso(), items.length, attachmentId);
    });
  }

  listAttachmentsForFulltextIndex({ statuses = ['pending', 'failed'], limit = 500 } = {}) {
    const allowed = statuses.filter(status => FULLTEXT_STATUSES.includes(status));
    if (!allowed.length) return [];
    const placeholders = allowed.map(() => '?').join(',');
    return this.db.prepare(`
      SELECT a.id, a.paper_id, a.relative_path, a.fts_status, a.fts_page_count, p.title AS paper_title
      FROM attachments a JOIN papers p ON p.id = a.paper_id
      WHERE a.fts_status IN (${placeholders})
      ORDER BY a.created_at ASC
      LIMIT ?
    `).all(...allowed, Math.max(1, Math.min(Number(limit) || 500, 2000)))
      .map(row => ({ ...row, absolutePath: this.resolveAttachmentPath(row.relative_path) }));
  }

  fulltextIndexStatus() {
    const rows = this.db.prepare("SELECT COALESCE(fts_status, 'pending') AS status, COUNT(*) AS count FROM attachments GROUP BY COALESCE(fts_status, 'pending')").all();
    const counts = { pending: 0, ready: 0, empty: 0, failed: 0 };
    let total = 0;
    for (const row of rows) {
      const count = Number(row.count || 0);
      if (counts[row.status] === undefined) { total += count; continue; }
  ...
  deleteFulltextForAttachments(attachmentIds) {
    const ids = [...new Set((Array.isArray(attachmentIds) ? attachmentIds : []).filter(Boolean))];
    if (!ids.length) return;
    const placeholders = ids.map(() => '?').join(',');
    this.db.prepare(`DELETE FROM papers_fts WHERE attachment_id IN (${placeholders})`).run(...ids);
  }

  deleteFulltextForPapers(paperIds) {
    const ids = [...new Set((Array.isArray(paperIds) ? paperIds : []).filter(Boolean))];
    if (!ids.length) return;
    const placeholders = ids.map(() => '?').join(',');
    this.db.prepare(`DELETE FROM papers_fts WHERE paper_id IN (${placeholders})`).run(...ids);
  }

  searchFullText({ query, projectId = null, limit = 20, matchesPerPaper = 3 } = {}) {
    const clean = String(query || '').trim().slice(0, 200);
    if (!clean) return { query: clean, hits: [], total: 0 };
    const matchQuery = buildMatchQuery(clean);
    if (!matchQuery) return { query: clean, hits: [], total: 0 };
    const cap = Math.max(1, Math.min(Number(limit) || 20, 50));
    const perPaper = Math.max(1, Math.min(Number(matchesPerPaper) || 3, 10));
    const conditions = ['papers_fts MATCH ?'];
    const params = [matchQuery];
    if (projectId) {
      conditions.push('paper_id IN (SELECT paper_id FROM project_papers WHERE project_id = ?)');
      params.push(projectId);
    }
    const rows = this.db.prepare(`
      SELECT paper_id, attachment_id, page, snippet(papers_fts, 0, '⟦', '⟧', '…', 14) AS snippet
      FROM papers_fts WHERE ${conditions.join(' AND ')}
      ORDER BY rank LIMIT ?
    `).all(...params, cap * 6);
    const byPaper = new Map();
    for (const row of rows) {
      const key = row.paper_id;
      if (!byPaper.has(key)) byPaper.set(key, { attachmentId: row.attachment_id, matches: [] });
      const hit = byPaper.get(key);
      if (hit.matches.length < perPaper) {
        hit.matches.push({ page: Number(row.page || 0), snippet: restoreCjkSpacing(row.snippet || '') });
      }
    }
    const paperIds = [...byPaper.keys()].slice(0, cap);
    if (!paperIds.length) return { query: clean, hits: [], total: 0 };
    const placeholders = paperIds.map(() => '?').join(',');
    const paperRows = this.db.prepare(`SELECT id, title, venue, year, doi FROM papers WHERE id IN (${placeholders})`).all(...paperIds);
    const paperMap = new Map(paperRows.map(row => [row.id, row]));
    const projectStmt = this.db.prepare(`
      SELECT pp.project_id, pr.title AS project_title
      FROM project_papers pp JOIN projects pr ON pr.id = pp.project_id
      WHERE pp.paper_id = ? AND pr.archived_at IS NULL
      ORDER BY pr.created_at ASC LIMIT 1
    `);
    const hits = paperIds.map(id => {
      const info = byPaper.get(id);
      const paper = paperMap.get(id) || {};
      const project = projectStmt.get(id) || null;
      return {
        paperId: id,
        title: paper.title || '',
        venue: paper.venue || '',
    ...
    return { query: clean, hits, total: hits.length };
  }

  // ── 引用核验（v22：汇总笔记引文四态账本；核验是审计，不是改写） ──

  replaceCitationVerifications(noteDocumentId, results) {
    const doc = this.db.prepare('SELECT id FROM paper_note_documents WHERE id = ?').get(noteDocumentId);
    if (!doc) throw new ResearchStoreError('NOTE_DOCUMENT_NOT_FOUND', '汇总笔记不存在', 404);
    const ownedIds = new Set(this.db.prepare('SELECT id FROM note_citations WHERE note_document_id = ?').all(noteDocumentId).map(row => row.id));
    const stamp = nowIso();
    const accepted = (Array.isArray(results) ? results : []).filter(item => item?.citationId && ownedIds.has(String(item.citationId)) && item.paperId);
    withTransaction(this.db, () => {
      for (const item of accepted) {
        this.db.prepare(`
          INSERT INTO citation_verifications(id, citation_id, paper_id, status, conflicts_json, sources_json, checked_at, created_at)
          VALUES(?, ?, ?, ?, ?, ?, ?, ?)
          ON CONFLICT(citation_id) DO UPDATE SET
            paper_id = excluded.paper_id, status = excluded.status, conflicts_json = excluded.conflicts_json,
            sources_json = excluded.sources_json, checked_at = excluded.checked_at
        `).run(crypto.randomUUID(), String(item.citationId), String(item.paperId), String(item.status),
          JSON.stringify(item.conflicts || []), JSON.stringify(item.sources || []), stamp, stamp);
      }
    });
    this.#audit('citation.verify', 'note_document', noteDocumentId, { count: accepted.length });
    return this.listCitationVerifications(noteDocumentId);
  }

  listCitationVerifications(noteDocumentId) {
    return this.db.prepare(`
      SELECT cv.id, cv.citation_id, cv.paper_id, cv.status, cv.conflicts_json, cv.sources_json, cv.checked_at
      FROM citation_verifications cv
      JOIN note_citations nc ON nc.id = cv.citation_id
      WHERE nc.note_document_id = ?
      ORDER BY cv.checked_at DESC
    `).all(noteDocumentId).map(row => {
      let conflicts = [];
      let sources = [];
      try { conflicts = JSON.parse(row.conflicts_json || '[]'); } catch { conflicts = []; }
  ...
  deleteAnnotation(projectId, annotationId) {
    ...
    this.#scheduleProjectNotesSync(projectId);
  ...
  renameNoteTag(oldTag, newTag) {
    ...
    // P0-4：JSON 片段 LIKE 预过滤把三张表的全表扫描缩到候选行（JS 端 includes 仍是精确判定）；
    // 含引号/反斜杠/LIKE 通配符的标签无法安全构造片段模式，退回全表（罕见路径）。
    const likeSafe = !/["\\%_]/.test(from);
    const tagFilter = likeSafe ? ` WHERE tags_json LIKE '%"${from}"%'` : '';
    const notes = this.db.prepare(`SELECT id, project_id, tags_json, annotation_id FROM notes${tagFilter}`).all();
    const sentenceNotes = this.db.prepare(`SELECT id, tags_json FROM sentence_notes${tagFilter}`).all();
    const noteDocs = this.db.prepare(`SELECT id, tags_json FROM paper_note_documents${tagFilter}`).all();
    // 批量预取关联批注，消除逐条查询（N+1）
    const annotationMap = new Map();
    const matchedAnnotationIds = notes
      .filter(note => note.annotation_id && parseJsonArray(note.tags_json).includes(from))
      .map(note => note.annotation_id);
    if (matchedAnnotationIds.length) {
      const placeholders = matchedAnnotationIds.map(() => '?').join(',');
      for (const row of this.db.prepare(`SELECT * FROM annotations WHERE id IN (${placeholders})`).all(...matchedAnnotationIds)) {
        annotationMap.set(row.id, row);
      }
    }
    let affected = 0;
    withTransaction(this.db, () => {
      ...
      for (const note of notes) {
        ...
        if (note.annotation_id) {
          const annotation = annotationMap.get(note.annotation_id);
          if (annotation) {
            try {
              const payload = JSON.parse(annotation.payload_json || '{}');
  ...
  /**
   * 项目笔记 md 的延迟合并写：把同一时间窗内的多次笔记变更合并为一次整文件落盘。
   * 读取路径（getProjectNotesFile）仍走强制同步，外部消费不会读到过期内容。
   */
  #scheduleProjectNotesSync(projectId) {
    this.#pendingNotesSync.add(projectId);
    if (this.#notesSyncTimer) return;
    this.#notesSyncTimer = setTimeout(() => {
      this.#notesSyncTimer = null;
      const ids = [...this.#pendingNotesSync];
      this.#pendingNotesSync.clear();
      for (const id of ids) {
        if (!this.db) return; // 实例已关闭
        try {
          this.syncProjectNotesFile(id);
        } catch (error) {
          console.warn('[hana-research] 项目笔记文件延迟同步失败:', error?.message || error);
        }
      }
    }, 300);
    if (typeof this.#notesSyncTimer.unref === 'function') this.#notesSyncTimer.unref();
  }
  ...
  listDuplicateCandidates({ projectId = null, includeIgnored = false, limit = 100 } = {}) {
    ...
    // 1) DOI 分桶：两条均有 DOI 的文献只可能在同桶内成对（规范化 DOI 不同即排除）。
    //    原实现对该象限做全量两两比较，是万级库卡顿的主因。
    const doiBuckets = new Map();
    for (const item of prepared) {
      if (!item.doi) continue;
      const bucket = doiBuckets.get(item.doi);
      if (bucket) {
        for (const other of bucket) evaluatePair(other, item);
        bucket.push(item);
      } else {
        doiBuckets.set(item.doi, [item]);
      }
    }
    // 2) 标题路径：仅"至少一条无 DOI"的组合需要（双有 DOI 已由分桶覆盖或按规则排除）。
    //    复杂度从 O(n²) 降为 O(|无DOI| × n)，无 DOI 文献通常是少数。
    const noDoiPositions = [];
    prepared.forEach((item, index) => { if (!item.doi) noDoiPositions.push(index); });
    // 左侧无 DOI × 右侧任意
    for (const leftIndex of noDoiPositions) {
      const left = prepared[leftIndex];
      for (let rightIndex = leftIndex + 1; rightIndex < prepared.length; rightIndex += 1) {
        evaluatePair(left, prepared[rightIndex]);
      }
    }
    // 左侧有 DOI × 右侧无 DOI（与上一循环互补，不重复成对）
    for (const rightIndex of noDoiPositions) {
      const right = prepared[rightIndex];
      for (let leftIndex = 0; leftIndex < rightIndex; leftIndex += 1) {
        const left = prepared[leftIndex];
        if (left.doi) evaluatePair(left, right);
      }
    }
    ...
    const summary = withTransaction(this.db, () => {
      ...
      for (const attachment of sourceAttachments) {
        ...
        if (existing) {
          ...
          this.deleteFulltextForAttachments([attachment.id]);
          attachmentsReused += 1;
        } else {
          this.db.prepare('UPDATE attachments SET paper_id = ? WHERE id = ?').run(targetId, attachment.id);
          // 全文索引跟着附件走：直接改 paper_id，避免整篇重提取
          this.db.prepare('UPDATE papers_fts SET paper_id = ? WHERE attachment_id = ?').run(targetId, attachment.id);
          attachmentsMoved += 1;
        }
      }
  ...
  #clearPaperMergeRows(targetId, sourceId) {
    ...
    this.deleteFulltextForPapers(ids);
    this.deleteFulltextForAttachments(attachmentIds);
  ...
  close() {
    ...
    if (this.#notesSyncTimer) {
      clearTimeout(this.#notesSyncTimer);
      this.#notesSyncTimer = null;
    }
    this.#pendingNotesSync.clear();
    if (this.db) {
...
function attachmentRow(row) {
  ...
  return {
    ...
    ftsStatus: row.fts_status || 'pending',
    ftsIndexedAt: row.fts_indexed_at || null,
    ftsPageCount: row.fts_page_count == null ? null : Number(row.fts_page_count),
    createdAt: row.created_at,
  };
}
