...
const seededProjects = [
  {
    id: 'project-adolescent-emotion',
    title: '青少年情绪应对',
    description: '关键事件访谈、情绪调节与情境化测评材料。',
    color: '#9b694d',
  },
  {
    id: 'project-chat-sjt',
    title: 'Chat-SJT 开发',
    description: '情境判断测验的证据、条目版本与效度研究。',
    color: '#607966',
  },
];
...
export class ResearchStore {
  ...
  deleteProject(projectId) {
    ...
    withTransaction(this.db, () => {
      this.db.prepare('DELETE FROM note_citations WHERE paper_id IN (SELECT paper_id FROM project_papers WHERE project_id = ?)').run(projectId);
      this.db.prepare('DELETE FROM paper_note_documents WHERE paper_id IN (SELECT paper_id FROM project_papers WHERE project_id = ?)').run(projectId);
      this.db.prepare('DELETE FROM paper_reading_state WHERE paper_id IN (SELECT paper_id FROM project_papers WHERE project_id = ?)').run(projectId);
      // v13：逐句笔记随项目级联（笔记为项目证据链一部分）
      this.db.prepare('DELETE FROM sentence_notes WHERE paper_id IN (SELECT paper_id FROM project_papers WHERE project_id = ?)').run(projectId);
      this.db.prepare('DELETE FROM projects WHERE id = ?').run(projectId);
    });
    // 项目笔记汇总文件
    const notesFile = path.join(this.projectNotesDir, projectId, 'project-notes.md');
    try {
