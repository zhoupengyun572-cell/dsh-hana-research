 ...
 */
export async function syncJournalSource({ store, source, daysBack = DEFAULT_DAYS_BACK, perPage = DEFAULT_PER_PAGE, allowedPdfHosts = [], provider = 'openalex', fromDate = null }) {
  ...
  const existingDois = store.listPaperDois();
  const existingIds = store.listPaperIds();
  const paperByDoi = new Map(
    store.listPapers()
      .filter(paper => paper.doi)
      .map(paper => [paper.doi, paper]),
  );
...
export async function syncOneJournalSource({ store, source, daysBack = DEFAULT_DAYS_BACK, allowedPdfHosts = [], rateLimitRetryMs = RATE_LIMIT_RETRY_MS }) {
  if (!source) throw new LiteratureSearchError('JOURNAL_SOURCE_NOT_FOUND', '期刊源不存在', 404);
  const startedAt = new Date().toISOString();
  try {
    let outcome;
    try {
      outcome = await syncJournalSource({ store, source, daysBack, allowedPdfHosts, provider: 'openalex' });
    } catch (error) {
      ...
      try {
        ...
        outcome = await syncJournalSource({ store, source, daysBack, allowedPdfHosts, provider: 'crossref' });
      }
    }
...
export async function syncAllJournals({ store, daysBack = DEFAULT_DAYS_BACK, allowedPdfHosts = [], rateLimitRetryMs = RATE_LIMIT_RETRY_MS }) {
  ...
  for (let index = 0; index < sources.length; index += 1) {
    ...
    results.push(await syncOneJournalSource({ store, source, daysBack, allowedPdfHosts, rateLimitRetryMs }));
