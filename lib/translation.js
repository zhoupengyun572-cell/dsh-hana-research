...
/**
 * 段级缓存：进程内存为准，防抖落盘。
 * 原实现每段翻译都整读+整写 translation-cache.json（最多 2000 条），长文档是 O(n²) 文件 IO；
 * 现在首次访问加载一次，写入只更新内存并在 500ms 防抖后合并落盘（TTL 清理 + 截断仍在落盘时做）。
 * 代价：进程退出瞬间未落盘的最后几段会丢缓存（下次重译该段），对缓存可接受。
 */
const translationCacheStates = new Map(); // dataDir -> { entries: Map, dirty, timer }
const TRANSLATION_CACHE_FLUSH_MS = 500;

function translationCacheState(dataDir) {
  let state = translationCacheStates.get(dataDir);
  if (!state) {
    const entries = new Map();
    try {
      const raw = JSON.parse(fs.readFileSync(cacheFilePath(dataDir), 'utf-8'));
      for (const [key, entry] of Object.entries(raw)) entries.set(key, entry);
    } catch { /* 文件缺失或损坏：从空缓存开始 */ }
    state = { entries, dirty: false, timer: null };
    translationCacheStates.set(dataDir, state);
  }
  return state;
}

export function flushTranslationCache(dataDir) {
  const state = translationCacheStates.get(dataDir);
  if (!state?.dirty) return;
  const now = Date.now();
  const fresh = [...state.entries.entries()].filter(([, entry]) => (
    entry?.translated && entry?.ts && now - entry.ts < CACHE_TTL_MS
  ));
  fresh.sort((a, b) => b[1].ts - a[1].ts);
  const pruned = Object.fromEntries(fresh.slice(0, CACHE_MAX_ENTRIES));
  ...
  state.entries = new Map(Object.entries(pruned));
  state.dirty = false;
}
...
export function cachedTranslation(dataDir, text) {
  ...
  const entry = translationCacheState(dataDir).entries.get(key);
...
export function storeTranslationCache(dataDir, key, translated) {
  const state = translationCacheState(dataDir);
  state.entries.set(key, { translated: String(translated || ''), ts: Date.now() });
  state.dirty = true;
  if (state.timer) clearTimeout(state.timer);
  state.timer = setTimeout(() => {
    state.timer = null;
    try { flushTranslationCache(dataDir); } catch { /* 落盘失败不中断翻译 */ }
