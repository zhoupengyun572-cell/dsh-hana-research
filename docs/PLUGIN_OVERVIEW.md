...
> 最后一版更新：2026-08-28（社区发布准备基线 0.5.0-beta.1）。
...
| 数据库 | SQLite WAL，schema 版本 **v22**（`$DSH_HOME/plugin-data/hana-research/research.db`） |
| 前端资产版本 | 与 `HANA_RELEASE_VERSION` 共用 **0.5.0-beta.1**（`lib/pages.js`，升级资源强制刷新缓存） |
...
│                 register-tools.js(21 个 Agent 工具) · reporting.js(订阅汇报) · client.js(桥接)
...
| Agent 协作 | 各处「交给 Agent」 + 副驾驶 | 21 个工具；只读可主动用，写入需确认+宿主审批+审计；接力进草稿由用户发送；project-overview/brief/news cards |
...
## 8. Agent 工具与安全模式（21 个）

- 只读：get_research_context / get_project_brief / list_research_projects / list_project_tasks / search_literature / search_project_papers / read_project_notes / list_tags / list_subscriptions / get_reader_context / report_journal_updates / get_ai_screening
